  ...
  useEffect(() => {
    queueMicrotask(() => {
      try {
        ...
        setData(typeof parsed.atsScore === "number" ? parsed : null);
      } catch {
        setData(null);
      }
    });
    ...
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-lg flex-col justify-center px-4 py-20 text-center">
        ...
        <p className="mt-4 text-zinc-400">
          Upload your resume from the home page to generate a report.
        </p>
        <Link
          href="/#scan"
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
      ...
      <div className="mt-10 flex flex-col gap-4 sm:flex-row">
        <Link
          href="/#pricing"
          className="inline-flex flex-1 justify-center rounded-full bg-emerald-500 px-6 py-3 text-center font-bold text-zinc-950 hover:bg-emerald-400"
        >
          Start interview sprint — $29
        </Link>
        <Link
          href="/"
          className="inline-flex flex-1 justify-center rounded-full border border-white/20 py-3 font-semibold text-white hover:bg-white/5"
        >
          Back to home
        </Link>
      </div>
